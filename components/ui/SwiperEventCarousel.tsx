...
export function SwiperEventCarousel({ events }: SwiperEventCarouselProps) {
  ...
  return (
    <div className="w-full relative">
      ...
      >
        {events.map((event) => (
          <SwiperSlide key={event.id} className="pb-4">
            <Card className="bg-white shadow-[0_4px_10px_rgba(0,0,0,0.1)] hover:shadow-[0_8px_16px_rgba(0,0,0,0.12)] transition-shadow rounded-2xl h-[420px] flex flex-col overflow-hidden border border-gray-100">
              ...
              {/* Bild mit Gradient-Overlay und Titel darüber */}
              <div className="relative h-64 flex-shrink-0 overflow-hidden">
                ...
                />
                {/* Gradient von unten für Lesbarkeit */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" />

                {/* Titel + Datum über dem Bild */}
                <div className="absolute bottom-0 left-0 right-0 px-4 pb-3 pt-8 text-white">
                  <div className="flex items-end justify-between gap-2">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base font-bold leading-snug drop-shadow-sm">{event.title}</h3>
                      <p className="text-xs text-white/85 mt-1 drop-shadow-sm">
                        {formatDate(event.date)} · {event.time}
                      </p>
                      <p className="text-xs text-white/70 mt-0.5 drop-shadow-sm leading-tight">{event.location}</p>
                    </div>
